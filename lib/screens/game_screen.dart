...
/// Bir küpün kaldırılma işlemini tutar - Geri Al butonunun bunu geri
/// alabilmesi için. `wentToTray` true ise küp tepsiye gitmişti (ve
/// varsa oluşturduğu eşleşme de birlikte geri alınır); false ise
/// bomba ile doğrudan yok edilmişti (tepsiyle hiç ilgisi yok).
class _RemovalAction {
  _RemovalAction({required this.cube, required this.wentToTray, this.matchedTriple});
  final _Cube cube;
  final bool wentToTray;
...
class _GameScreenState extends State<GameScreen> {
  ...
  static const double _tileHeight = 26;
  static const double _cubeDepth = 32;
  ...
  final List<_RemovalAction> _removalHistory = [];
  ...
  void _generateLevel() {
    // Klasik piramit: her katman bir alttakinden TAM 1 birim küçük, her
    // üst küp altındaki 4 küpün TAM ortasına (yarım hücre kaymış) oturur
    // - referans görseldeki gibi yumuşak, daralan bir piramit oluşur.
    _n = (2 + _level).clamp(3, 6);
  ...
  /// Bir küp, üzerinde oturan (en fazla 4 tane olabilen) üst katman
  /// küplerinin HEPSİ kaldırılmışsa açığa çıkar - klasik piramit kuralı.
  ...
  /// Bomba, bloğu tepsiye GÖNDERMEZ - doğrudan yok eder. Herhangi bir
  /// göreve/sete katkısı olmaz, sadece yoldan çekmek için kullanılır.
  /// Kapalı (üstü örtülü) küplerde bile çalışır.
  void _destroyWithBomb(_Cube cube) {
    setState(() {
      cube.removed = true;
      _removalHistory.add(_RemovalAction(cube: cube, wentToTray: false));
    });
    _vibrate(50);
  }
  ...
  void _sendToTray(_Cube cube) {
    setState(() {
      ...
      _removalHistory.add(_RemovalAction(cube: cube, wentToTray: true, matchedTriple: matched));
  ...
  void _undo() {
    ...
    setState(() {
      ...
      if (action.wentToTray) {
        if (action.matchedTriple != null) {
          for (final c in action.matchedTriple!) {
            _tray.add(c);
          }
          for (final mission in _missions) {
            if ((mission.isGeneral || mission.color == action.cube.color) &&
                mission.progress > 0) {
              mission.progress--;
            }
          }
        }
        _tray.remove(action.cube);
      }
      action.cube.removed = false;
      _undoCount--;
    });
  }
  ...
  Offset _cubeAnchor(_Cube cube, Size canvasSize) {
    // Her katman kendi boyutuna göre ortalanır - üst katmanlar bir
    // alttakinden 1 küçük olduğu için, izometrik izdüşümde otomatik
    // olarak altındaki 4 küpün TAM ORTASINA (yarım hücre kaymış) oturur.
    // Bu, referans görseldeki yumuşak/daralan piramit görünümünü verir.
  ...
  ///
  /// KESİN KURAL: bir üst katmandaki küp, HER ZAMAN altındaki katmandan
  /// SONRA (yani önünde/üstünde) çizilmeli - bu, piramit döndürülse bile
  /// asla değişmeyen fiziksel bir gerçek (dikey eksende döndürme, hangi
  /// katmanın hangisinin üstünde olduğunu değiştirmez). Bu yüzden birincil
  /// sıralama katman numarasına göre; aynı katman içindeki küpler zaten
  /// birbirini örtmüyor (izometrik ızgara kusursuz döşenir), o yüzden
  /// aralarındaki sıra önemsiz - sadece tutarlılık için ekran Y'sine göre
  /// ikincil sıralanıyor.
